...
推荐入口：`core/multimodal_multiagent/orchestrator.py`

协议与文档：

- `core/multimodal_multiagent/contracts.py`
- `docs/ARCHITECTURE_MULTIMODAL_MULTIAGENT.md`
...
```bash
# 方式 1：统一列表（按顺序回退）
export LLM_API_KEYS="sk-primary...,nvapi-backup..."
export LLM_PROVIDERS="openai,nvidia"
export LLM_MODELS="gpt-5-mini,meta/llama-3.1-70b-instruct"
export LLM_BASE_URLS="https://api.openai.com/v1,https://integrate.api.nvidia.com/v1"

# 方式 2：保留主配置 + 追加备份
export LLM_API_KEY="sk-primary..."
export LLM_MODEL="gpt-5-mini"
export LLM_BACKUP_API_KEYS="sk-backup...,nvapi-backup..."
export LLM_BACKUP_MODELS="gpt-5-nano,meta/llama-3.1-70b-instruct"
```

当当前 API 调用失败时会自动切换到下一条配置；全部不可用时，会最终回退到本地 ModelScope 0.5B 模型（默认 `Qwen/Qwen2.5-0.5B-Instruct`）。

本地兜底可选配置：

```bash
export LLM_ENABLE_LOCAL_FALLBACK=1
export LLM_LOCAL_MODEL_ID="Qwen/Qwen2.5-0.5B-Instruct"
export LLM_LOCAL_CACHE_DIR="models/modelscope"
export LLM_LOCAL_MAX_NEW_TOKENS=256
```

若你希望关闭本地兜底并在 API 全失效后直接报错：

```bash
export LLM_ENABLE_LOCAL_FALLBACK=0
```
